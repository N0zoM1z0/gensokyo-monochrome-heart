...
func _finish(failures: Array[String]) -> void:
	print("M14 Aya Camera Down integration: failures=%d" % failures.size()); for failure: String in failures: printerr("FAIL: %s" % failure); quit(0 if failures.is_empty() else 1)
