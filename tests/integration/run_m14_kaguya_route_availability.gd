extends SceneTree
## Proves Kaguya's authored route stages unlock in order.

const EVENTS: Array[StringName] = [
	&"evt.ein.five_impossibilities",
	&"evt.ein.elegance_and_boredom",
	&"evt.ein.game_with_no_continue",
...
func _initialize() -> void:
	if not _content.load_sources().is_success():
		_finish(["Kaguya route content could not load"])
		return
...
func _expect_gate(evaluator: EventPredicateEvaluator, event_id: StringName, index: int) -> void:
	...
	var state := _state(StringName("p184%d" % index))
	if index == 0:
		_expect(evaluator.all_pass(evaluator.evaluate_all(graph.availability, state)), "Kaguya's opening event was unavailable")
		return
...
func _state(profile_id: StringName) -> GameState:
	...
	var state := GameStateFactory.create_new(profile_id, characters, locations, 1841)
...
func _finish(failures: Array[String]) -> void:
	print("M14 Kaguya route availability integration: failures=%d" % failures.size())
