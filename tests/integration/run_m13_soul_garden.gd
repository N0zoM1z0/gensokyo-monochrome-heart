...
func _run() -> void:
	...
	_expect(mode.garden.is_paused, "Soul Garden did not visibly pause before offering withdrawal")
	mode.handle_semantic_action(GameInput.CANCEL)
	_expect(mode.final_result != null and mode.final_result.result_tag == &"withdrawn", "Soul Garden pause Cancel did not create the voluntary withdrawal result")
	mode.handle_semantic_action(GameInput.CONFIRM)
	_expect(_completion_count == 2, "Soul Garden did not hand off the voluntary withdrawal exactly once")
