...
OUTPUT_DIR="${GMH_RELEASE_OUTPUT:-build/release-candidate/gensokyo-monochrome-heart-${RELEASE_VERSION}-linux-x86_64}"
...
mkdir -p "$OUTPUT_DIR"
"$GODOT_BIN" --headless --path . --export-release "Release Linux" "$OUTPUT_DIR/$PACKAGE_NAME"
[[ -s "$OUTPUT_DIR/$PACKAGE_NAME" && -s "$OUTPUT_DIR/${PACKAGE_NAME%.x86_64}.pck" ]] || fail "export did not create the Linux binary and PCK"
...
python3 - "$OUTPUT_DIR" "$RELEASE_VERSION" "$CONTENT_REVISION" "$SAVE_SCHEMA" <<'PY'
import hashlib
import json
import subprocess
import sys
...
version, revision, save_schema = sys.argv[2:]
files = []
...
echo "Release candidate built and smoke-tested: $OUTPUT_DIR"
